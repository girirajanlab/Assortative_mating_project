...
df = pd.read_csv('tables/10_replace_escape.csv')

# No genes with multiple functions - good!
print(df[(df['Func.wgEncodeGencodeBasicV38'].str.contains(';')) & ~(df['Gene.wgEncodeGencodeBasicV38'].str.contains(';'))][['Func.wgEncodeGencodeBasicV38', 'Gene.wgEncodeGencodeBasicV38']])
...
print(df[(df['Gene.wgEncodeGencodeBasicV38'] != df['Gene_symbol'])][['Chr', 'Pos', 'Ref', 'Alt', 'Gene.wgEncodeGencodeBasicV38', 'Gene_symbol', 'Gene_id', 'Gene_biotype']])

# Filter again for only protein-coding genes
df=df[df.Gene_biotype.str.contains('protein_coding')]
print(df.shape)

df.to_csv('tables/11_gene_ids.csv', index=False)
